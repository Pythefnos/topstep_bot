...
class TopstepAPIMock:
    """
    A mock Topstep API for simulation mode.
    ...
    def __init__(self, initial_balance: float = 0.0, profit_per_trade: float = 0.0, symbol: str = "SIM"):
        ...
        self.order_count = 0
        self.symbol_id = symbol  # mimic a contract identifier
    ...
    def get_latest_price(self) -> float:
        """Return a simulated market price (monotonically increasing)."""
    ...
    def place_order(self, symbol: str, side: str, size: int) -> str:
        """Simulate placing a market order and 'fill' it with a fixed profit outcome."""
        self.order_count += 1
    ...
    def get_starting_balance(self) -> float:
        """Get the initial account balance (for parity with real API)."""
